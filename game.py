...
MAP = Entity(model='e1m1.obj', texture='sheet133_166-6.png')
MAP.collider = MeshCollider(MAP, center=(0, 0, 0))
player = FirstPersonController(model='cube', texture='2dassets/py.png', scale=(2, 2, 2), speed=20)
player.collider = BoxCollider(player, Vec3(2, 2, 2))
Text(text='Weapon: Gun1', position=(0, -.45, 0))

gun1 = Entity(model='3dassets/assaultrifle1', parent=camera, position=(.10, -.25, .50), rotation=(90, 90, 0), origin_z=-.5, color=color.red,
              on_cooldown=False)
gun1.muzzle_flash = Entity(parent=gun1, world_scale=.5, model='quad', color=color.yellow, enabled=False)
